...
async fn upload_image(
    ...
    while let Some(field) = multipart
        .next_field()
        .await
        .map_err(|e| ImageApiError::InvalidRequest(format!("Failed to process form: {e}")))?
    {
    ...
    let filename =
        filename.ok_or_else(|| ImageApiError::InvalidRequest("Missing filename".to_string()))?;

    let image_data = image_data
        .ok_or_else(|| ImageApiError::InvalidRequest("Missing image data".to_string()))?;
    ...
    let url = state
        .storage
        .get_image_url(&stored_filename, &state.base_url);
...
async fn get_image(
    ...
    // Get the image data
    let image_data = state.storage.get_image(&filename).await.map_err(|e| {
...
/// List all images
#[instrument(skip(state), err)]
async fn list_images(State(state): State<Arc<ImageApiState>>) -> ImageApiResult<impl IntoResponse> {
...
async fn delete_image(
    ...
    // Delete the image
    state.storage.delete_image(&filename).await.map_err(|e| {
        if e.to_string().contains("No such file") {
            ImageApiError::NotFound(format!("Image not found: {filename}"))
        } else {
            ImageApiError::StorageError(e)
...
pub fn create_image_api_router(base_url: &str) -> Router {
    ...
    let storage =
        Arc::new(ImageStorage::new(storage_config).expect("Failed to create image storage"));
